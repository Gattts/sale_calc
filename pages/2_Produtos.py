...
import time # <--- AQUI ESTAVA O ERRO (Faltava esse import)
    ...
    if len(df_filtrado) == 0:
        ...
        for sku in skus_unicos:
            ...
            with st.expander(f"📦 {sku} | {nome_prod} — Última: R$ {preco_recente:,.2f} ({data_recente})"):
                ...
                # --- ÁREA DE EXCLUSÃO (DENTRO DE UM CONTAINER VERMELHO) ---
                st.markdown('<div class="delete-section">', unsafe_allow_html=True)
                st.markdown("🗑️ **Zona de Perigo / Exclusão**")
                
                col_del1, col_del2 = st.columns(2)
                
                # OPÇÃO 1: APAGAR UM REGISTRO ESPECÍFICO (NF/DATA)
                with col_del1:
                    st.caption("Opção A: Apagar apenas uma entrada específica (ex: erro de digitação)")
                    ...
                    registro_para_apagar = st.selectbox("Selecione a entrada:", options=list(opcoes_exclusao.keys()), key=f"sel_{sku}")
                    
                    if st.button("Apagar Registro Selecionado", key=f"btn_del_one_{sku}"):
                ...
                # OPÇÃO 2: APAGAR O PRODUTO INTEIRO (TODOS OS REGISTROS COM ESSE SKU)
                with col_del2:
                    st.caption("Opção B: Excluir o produto e TODO seu histórico")
                    st.write("") # Espaço para alinhar botão
                    if st.button("💥 Excluir Produto Completo", key=f"btn_del_all_{sku}", type="primary"):
                        # Deleta tudo que tem esse SKU
